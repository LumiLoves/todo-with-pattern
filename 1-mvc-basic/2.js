...
class TodoController {
  ...
  init() {
    ...
    this.view.fnFormSubmit = this.onSubmitForm.bind(this);
  }
  onSubmitForm(todoText) {
    this.model.addTodo(todoText, [this.updateview, this.updatecountview]);
  ...
  updatecountview() {
    this.coutnvie.render(this.todo);
...
class ListView {
  ...
  handleFormEvent() {
    document.addEventListener('submit', (e) => {
      ...
      if (target === this.form) {
        e.preventDefault();
        todoText && this.fnFormSubmit(todoText);
      }
    });
  }
}
