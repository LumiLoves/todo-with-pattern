...
class TodoModel {
  ...
  addTodo(text, fnafteraddtodo, fnsbeforeaddtodo) {
    //if(fnbeforeaddtodo)fnbeforeaddtodo();
    this.todo.set(this.counter, { text });
    this.addCounter();
    //fnafteraddtodo(this.todo);
    fnsbeforeaddtodo.forEach(fn => fn());
  }
  getTodo(id) {
...
class TodoController {
  ...
  init() {
    ...
    if (!listLength && todoLength) { this.updateListView(this.model.todo); }
    this.view.onSubmitForm = this.handleSubmitForm.bind(this); // 이거!
  }
  handleSubmitForm(todoText) {
    this.model.addTodo(todoText, [ this.updateListView ]);
  ...
  // updateCountView() {
  //   this.countView.render(this.todo);
  // }
}
...
class ListView {
  ...
  handleFormEvent() {
    document.addEventListener('submit', (e) => {
      ...
      if (target === this.form) {
        e.preventDefault();
        todoText && this.onSubmitForm(todoText);
      }
    });
  }
}
